...
int main() {
  ...
  if (ml > 1) {
    for (auto e : p) {
      int t = upper_bound(m1.begin(), m1.end(), -e) - m1.begin();
      long long t1 = abs(m2[t].first + e), t2 = t1;
      if (t > 0) {
        t2 = abs(m2[t - 1].first + e);
      }
      if (mn < t1 && mn < t2) {
        continue;
      }
  ...
  if (pl > 1) {
    for (auto e : m) {
      int t = upper_bound(p1.rbegin(), p1.rend(), -e) - p1.rbegin();
      long long t1 = abs(p2[t].first + e), t2 = t1;
      if (t > 0) {
        t2 = abs(p2[t - 1].first + e);
      }
      if (mn < t1 && mn < t2) {
        continue;
      }
